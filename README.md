...
Также для исправления существующих и написания новых тестов был использован проект optimization-reference-app, который находится в папке reference_app.
...
```
Исправление было достаточно простым - вместо использования сырых указателей и ручного управления памятью, было решено просто вернуть значение 84 напрямую, через разыменование Box. После правки тест успешно проходит, а запуск с MIRI не выявляет больше ошибок использования после освобождения памяти.
Правка выполнена в коммите с SHA 3350b56139f15a3c77e1129b12ac297d33d51e38.
...
Результаты замера до оптимизаций и после оптимизаций доступны по [ссылке](artifacts/optimization_measurements/report/index.html) (открывать локально через браузер).